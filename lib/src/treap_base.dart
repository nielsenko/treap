...
final class TreapBase<T, NodeT extends Node<T, NodeT>> {
  final NodeT? _root;
  final NodeContext<T, NodeT> _ctx;
  ...
  factory TreapBase.of(Iterable<T> items, [Comparator<T>? compare]) {
    compare ??= Comparable.compare as Comparator<T>;
    final ctx = NodeContext<T, NodeT>(compare, _createNode);
    NodeT? root;
  ...
  TreapBase<T, NodeT> _new(NodeT? root) =>
      identical(root, _root) ? this : TreapBase._(root, _ctx);
  ...
  TreapBase<T, NodeT> add(T item) => _new(upsert(_root, item, false, _ctx));
  ...
  TreapBase<T, NodeT> addOrUpdate(T item) =>
      _new(upsert(_root, item, true, _ctx));
  ...
  @pragma('vm:prefer-inline')
  TreapBase<T, NodeT> remove(T item) => _new(erase(_root, item, _ctx));
  ...
  TreapBase<T, NodeT> take(int n) => _new(node.take(_root, n, _ctx));
...
typedef Treap<T> = TreapBase<T, Node<T, ImmutableNode<T>>>;
