...
class Treap<T extends Comparable<T>> {
  ...
  // Returns a new treap with the first n items. If n is greater than the size of the treap, returns the original treap.
  Treap<T> take(int n) =>
      n < size ? Treap._(_root.split(_root.select(n).item).low) : this;

  // Skips the first n items and returns a new treap with the remaining items. If n is greater than or equal to the size of the treap, returns an empty treap.
  Treap<T> skip(int n) {
  ...
  // Operator overloading for adding an item to the treap. Returns a new treap with the added item.
  Treap<T> operator +(T item) => add(item);

  // Operator overloading for the union of this treap and another treap. Returns a new treap that is the union of the two treaps.
  Treap<T> operator |(Treap<T> other) => union(other);

  // Operator overloading for the intersection of this treap and another treap. Returns a new treap that is the intersection of the two treaps.
  Treap<T> operator &(Treap<T> other) => intersection(other);

  // Operator overloading for the difference of this treap and another treap. Returns a new treap that is the difference of the two treaps.
  Treap<T> operator -(Treap<T> other) => difference(other);

  // Operator overloading for selecting an item in the treap by its rank. Returns the selected item.
